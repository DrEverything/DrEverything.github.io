<script lang="ts">
  ...
  async function saveProject() {
    try {
      if (editingProject) {
        ...
        await api("/project/add", projectForm);
  ...
  async function deleteProject(id: string) {
    try {
      await api("/project/delete", { project_id: id });
  ...
  async function loadTasks(projectId: string) {
    tasksLoading = true;
    try {
      tasks = await api("/task/list", { project_id: projectId });
    } catch {
  ...
  async function deleteTask(t: Task) {
    try {
      await api("/task/delete", { task_id: t.task_id });
  ...
  async function saveIdea() {
    try {
      if (editingIdea) {
        await api("/idea/update", {
          idea_id: editingIdea.idea_id,
          ...ideaForm,
        });
        toast.success("Idea updated");
      } else {
        await api("/idea/add", ideaForm);
  ...
  async function deleteIdea(id: string) {
    try {
      await api("/idea/delete", { idea_id: id });
